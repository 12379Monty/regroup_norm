...
subtitle: https://groupnorm.netlify.app/_gn4gd_analysis
...
```{r gn4gd-load-packages,  include=FALSE, echo=FALSE, results='hide', message=FALSE, cache=T}
...
required_packages <- c("limma", "reprocessCore", "ArrayExpress", "GenomicRanges",
...
* David 2006 [@David:2006aa] and Huber 2006 [@Huber:2006aa] offer an alternative  procedurure and provide data
...
* <span style="color: blue;">**Normalization**<span/>,: Correct for probe-specific effects (the focus of the Group Normalization method)
...
* **Comparativ Analysis**: Compare nucleosome positions between conditions
...
* depletion of signal at promoters (NDRs)
...
* For each probe on the array, reference sample probe intensities are interrogated to form 
a *reference probe set*: a set of N = 1000 (a parameter) probes having similar intensities 
in the reference as the index probe.

* Using the reference probe sets, a reference set of test sample probe intensities is constructed
for each probe on the array.

<p></p>
* With the reference set of test sample probe intensities define a mean for the
low signal probes, $\mu_{i, low}$, and a mean for the high signal probes, $\mu_{i, high}$
   - for the nucleosome samples,  $\mu_{i, low}$ is taken to bee the 30th percentile
and $\mu_{i, high}$ the 70th percentile of he reference set of test sample probe intensities.

* If the test set probe intensity of a probe is $value$, the 
<span style="color: blue;">**group normalized value**</span> is given 
by the following transformation:

\begin{equation}

 \text{normalized value} =  \frac{value - \mu_{low}}{\mu_{high} - \mu_{low}}
 \tag{1}
...
* Can this transformation be optimized?
   - would think that the closer $\mu_{low}$ is to the probe effect for each probe,
the better the correction would be 
   - expect $\mu_{low}$ to be low variance but potentially high bias.
   - the denominator is just a change of units and is probably not needed.
      - the only tasks in this case is determining the presence of Nucleosom or not.



<p><p/>
* <span style="color: blue;">**Quantile-based Group Normalization**:</span>, maps each probe
to its rank or percentile in the comparable distribution.
...
```{r, echo=F}
CAPTION <- readLines("_GN4GD_analysis_figures/Figure3A.txt", warn=F)
```
...
```{r gn4gd-Figure3A, fig.cap=CAPTION}

knitr::include_graphics("_GN4GD_analysis_figures/Figure3A.png", dpi=125)
...
<p><p/>
* The performance assessment used in the paper and described below is a 2 condition/4 samples metric.

<p><p/>
* The performance assessment used in the paper and described below is a 2 condition/4 samples metric.
* A preferable similar metric would be based on the S/N that is relevant in the within sample
segmentation.
- ie S/N stats computed based on contiguous and unambiguous segments.
...
* $\mu_{low}}$ can be set at a level where it is entirely determined by the reference probes
   - If fewer than 25% of the probes in a set are in the test condition, then Q1 is
entirely determined by the reference probes, which is what we want.

## Read in Data {-}

```{r ARCH-gn4gd-read-lee, cache=T, cache.vars = "XYZ", echo=F, eval=F}
...
```{r ARCH-gn4gd-load-lee-2, cache=T, cache.vars = "XYZ", echo=F, eval=F}
#ls_vec <- ls()
