...
* Group Normalization has the property that only probe intensities in the test samples
are used so that there is no need foor a global adjustment to correct for differeces between test 
and reference.

* The reference sample probe intensities are only used to select probes:
   - For each probe, find N (~1000) reference probes with similar values in the reference sample.
       - This forms a "comparable set of probes", one set for each probe.
       - The test sample probe intensities on the comparable probe set provides a reference distribution of
probe intensities for each test sample probe.

* The so-called <span style="color: blue;">binary group normalization<span/> method Maoanconverts the probe indneities into somethhing that resembles
a standard unit:   `normalized_value = (value - μ_low) / (μ_high - μ_low)`


* <span style="color: blue;">**Quantile-based Group Normalization**:<span/>, maps each probe
to its rank or pecenrtile in the comparable distribution.

* This is very similar to local regression analysis, loess. 
...
knitr::include_graphics("_GN4GD_analysis_figures/Figure3A.png", dpi=200)
...
# Data Analysis - Huber vs Beer ,,, TBC
