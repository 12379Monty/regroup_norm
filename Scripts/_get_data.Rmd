...
antisense targets were offset by 4bp</span>** to yield  a high resolution view
of the genome
...
all probes with reporterRefSeq in the index set (i.e. in 0-1-25~{xx}
...
```{r get-data-reps-by-reg, cache =T, cache.vars='', results='asis'} 
...
for(RN in unique(AE_array_plus_tibl$regName)) {
  ...
  dplyr::filter(probeSign == "0-1-25")

  all_index_reporterRefSeq_frm <- merge( 
  index_reps_frm %>% dplyr::select(regionName, reporterRefSeq),
  reg_array_plus_tibl)

  nreps_reporterRefSeq_frm <- all_index_reporterRefSeq_frm %>%
  dplyr::group_by( regionName, reporterRefSeq, sense) %>%
  dplyr::summarize(nreps = dplyr::n())

  nreps_reporterRefSeq_tbl <- with(nreps_reporterRefSeq_frm,
  table(regionName, pmin(16, nreps))
  )

 nreps_reporterRefSeq_tbl %>%
 knitr::kable(
...
#### Key Papers {-}
- Lee, W., Tillo, D., Bray, N., Morse, R.H., Davis, R.W., Hughes, T.R., and Nislow, C. (2007). A high-resolution atlas of nucleosome occupancy in yeast. Nature Genetics 39, 1235–1244. https://doi.org/10.1038/ng2117
