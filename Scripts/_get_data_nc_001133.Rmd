...
AE_array_frm %<>% 
dplyr::select(-V8) %>%
dplyr::rename(regionName = targetName)
...
* Of the `r nrow(AE_array_frm)` cells.
   ...
   - `r round(100*sum(!is.na(AE_array_frm$regionName))/nrow(AE_array_frm))`%
...
to yield  a high resolution view of the genome
...
```{r get-data-by-target, cache=T, cache.vars=c("AE_array_plus_frm"), eval=LONG_EVAL}
...
```{r}
loadObj("_get_data_AE_array_plus_frm", 'AE_array_plus_frm')
...
xxx  <- function() {
JJ <- 1
a <- nc_001133_frm$reporterRefSeq[JJ]
b <-  nc_001133_frm$reporterRefSeq[JJ+1]
}
...
-->
```{r get-data-by-target-2, cache=T, cache.vars=c("AE_array_plus_frm"), eval=R & LONG_EVAL}
 # DO NOT DIFF WITHIN STRAND
AE_array_plus_frm <- AE_array_plus_frm %>%
  dplyr::select(-c(probeSize, targetLength, 
                   reporterSequence, rcSeq)) %>%
  dplyr::group_by(regionName) %>%
  dplyr::arrange(regionName, targetPos) %>%
  dplyr::mutate(
    diffPos = targetPos - lag(targetPos),
    ###regionNum = rank(targetPos),  - NOT NEEDED
    lcs_result = purrr::map2(lag(reporterRefSeq), reporterRefSeq,
       find_lcs_str_f)
  ) %>%
...
```{r get-data-by-target-nc-112233, cache=T, cache.vars=c("nc_001133_array_plus_frm"), eval=R & LONG_EVAL}
 # DO NOT DIFF WITHIN STRAND
nc_001133_array_plus_frm <- AE_array_plus_frm %>%
  dplyr::select(-c(probeSize, targetLength, 
                   reporterSequence, rcSeq)) %>%
...
saveObj("_get_data_nc_001133_array_plus_frm", 'nc_001133_array_plus_frm')
```
...
AE_array_probe_counts_frm <- nc_001133_array_plus_frm %>%
dplyr::group_by(regionName, sense) %>%
dplyr::summarize(
   numTargets = dplyr::n_distinct(targetPos))
...
to see how well the probes on the two strands align.
   - note that there could be misalignment  and 
...
```{r get-data-nc-001133, cache=T, cache.vars=c('nc_001133_frm'), eval=LONG_EVAL}


nc_001133_frm <- AE_array_plus_frm %>%
dplyr::filter(regionName == "ref|NC_001133|") %>%
dplyr::select(Row, Column, reporterName, targetPos, sense,
diffPos, reporterRefSeq, rcSeq) %>%
